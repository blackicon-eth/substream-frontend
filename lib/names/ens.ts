...
import { ENS_PUBLIC_RESOLVER_ADDRESS, ENS_PUBLIC_RESOLVER_ADDRESS_2 } from "../constants";
import { env } from "../env";
...
export const viemMainnetPublicClient = createPublicClient({
  ...
  transport: http(
    `${AlchemyRpcBaseUrls[mainnet.name.toLowerCase() as keyof typeof AlchemyRpcBaseUrls]}/${
  ...
  try {
    const foundEns = await viemMainnetPublicClient.getEnsName({
      ...
      universalResolverAddress:
        resolver === ENS_PUBLIC_RESOLVER_ADDRESS_2 || resolver === ENS_PUBLIC_RESOLVER_ADDRESS
          ? ("" as `0x${string}`)
...
/**
 * Get the ENS avatar for a name
 * @param name - The name to get the ENS avatar for
 * @returns The ENS avatar for the name
 */
export const getEnsAvatar = async (name: string): Promise<string> => {
  const resolver = await getResolver(viemMainnetPublicClient, {
    name: normalize(name),
  });

  try {
    const foundEnsAvatar = await viemMainnetPublicClient.getEnsAvatar({
      name: normalize(name),
      universalResolverAddress:
        resolver === ENS_PUBLIC_RESOLVER_ADDRESS_2 || resolver === ENS_PUBLIC_RESOLVER_ADDRESS
          ? ("" as `0x${string}`)
    ...
    return foundEnsAvatar ? (foundEnsAvatar as string) : "";
  } catch (error) {
    console.log("Error in getting ENS avatar", error);
    return "";
  }
};
 ...
 */
export const getAddressFromEns = async (domain: string): Promise<Address | null> => {
